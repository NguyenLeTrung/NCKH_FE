...
@Component({
  selector: 'kt-import-classroom',
  templateUrl: './import-classroom.component.html',
  styleUrls: ['./import-classroom.component.scss']
})
export class ImportClassroomComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<ImportClassroomComponent>,
  ...
  downloadSampleFile(){
    this.classroomService.dowloadFileTemplate('DS_Lophoc.xlsx');
  }
  onFileInput(event){
  ...
  importFile(){
    this.classroomService.importClassroom(this.fileImport, this.typeImport).subscribe((res: any) =>{
      if (res.data != null) {
  ...
  downloadErrorFile() {
    ...
    if (this.resultImport.listErrors.length > 0) {
      ...
      this.classroomService.exportDataErrors(this.resultImport.listErrors, nameFile);
    } else {
